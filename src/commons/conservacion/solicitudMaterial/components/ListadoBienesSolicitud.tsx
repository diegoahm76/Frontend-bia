...
import { set_bien_selected, set_bienes_solicitud_aux } from "../../distribucion/store/slice/distribucionSlice";
...
const ListadoBienesSolicitud = () => {
    ...
    const { bienes_solicitud } = useAppSelector((state) => state.solicitud_vivero);
    const { bienes_solicitud_aux, bienes_despacho } = useAppSelector((state) => state.distribucion);
            ...
            if(bienes_despacho.length > 0){
                ...
                bienes_solicitud.forEach((option) => {
                    if(option.cod_tipo_elemento_vivero === "IN"){
                        ...
                        aux_items.push({...option, cantidad_despachada: bien?.cantidad_despachada, cantidad_faltante: (option.cantidad??0) - (bien?.cantidad_despachada??0)})
                    } else {
                        bienes_despacho.forEach((option_despacho)=>{
                            if(option.id_bien === option_despacho.id_bien){
                                despachada = despachada + (option_despacho.cantidad_despachada??0)
                            }
                        })
                        aux_items.push({...option, cantidad_despachada: despachada, cantidad_faltante: (option.cantidad??0) - despachada})
                    }
                })
    ...
    const select_model = (): void => {
        const model = bienes_solicitud_aux.find((p) => p.id_bien === selected_row[0])
        if (model !== undefined) {
