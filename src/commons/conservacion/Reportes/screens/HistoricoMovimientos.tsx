...
import {
  historico_bajas,
  historico_cambios_etapa,
  historico_distribuciones,
  historico_ingreso_cuarentena,
  historico_levantamiento_cuarentena,
  historico_siembras,
  historico_traslados,
} from '../thunks/HistoricoMovimientos';
import { DialogNoticacionesComponent } from '../../../../components/DialogNotificaciones';
import ReportesXLS from './reportesXLS';

const lista_reporte = [
  {
    name: 'Movimientos de Bajas de Herramientas, Insumos y Semillas',
    value: 'MHIS',
  },
  { name: 'Distribución de Despachos Entrantes a Viveros', value: 'DDEV' },
  { name: 'Registros de Siembras', value: 'RES' },
  { name: 'Cambio de Etapa de Material Vegetal', value: 'CEMV' },
  { name: 'Ingreso a Cuarentena de Material Vegeta', value: 'ICMV' },
  { name: 'Levantamiento de Cuarentena de Material Vegetal', value: 'LCMV' },
  { name: 'Traslados Entre Viveros', value: 'TEV' },
];
...
export const HistoricoMovimientosScreen: React.FC = () => {
  ...
  const [reporte_consolidado, set_reporte_consolidado] =
  ...
  const obtener_viveros_fc: () => void = () => {
    dispatch(obtener_viveros()).then((response: any) => {
      const viveros_activos = response.data.filter(
  ...
  const cambio_seleccion_vivero: (event: SelectChangeEvent) => void = (
    e: SelectChangeEvent
  ) => {
    set_seleccion_vivero(e.target.value);
  };
  const cambio_reporte: (event: SelectChangeEvent) => void = (
    e: SelectChangeEvent
  ) => {
  ...
  const descargar_pdf = () => {
    ...
    doc.save(
      `${
        titulo_reporte.reporte_seleccionado !== null &&
        titulo_reporte.reporte_seleccionado !== undefined
          ? titulo_reporte.reporte_seleccionado.name
          : ''
      }.pdf`
    );
  };
  ...
  const crear_encabezado: () => {
    reporte_seleccionado: any;
    title: string;
  } = () => {
    const reporte_seleccionado = lista_reporte.find(
      (r: any) => r.value === seleccion_reporte
    );
    const title = `${
      reporte_seleccionado !== null && reporte_seleccionado !== undefined
        ? reporte_seleccionado.name
        : ''
    }`;
    ...
    const fechas = `${dayjs(fecha_desde).format('DD/MM/YYYY')} - ${dayjs(
      fecha_hasta
    ).format('DD/MM/YYYY')}`;
    if (planta !== '') {
      doc.text(
  ...
  const generar_historico_bajas: () => void = () => {
    const reporte_titulo: { reporte_seleccionado: any; title: string } =
      crear_encabezado();
    ...
    reporte.forEach((report: any) => {
      ...
      coordenada_y =
        reporte_dos.length === 1
          ? 45 + coordendas + 20
          : reporte_dos.length === 2
          ? 45 + coordendas + 30
          : 45 + coordendas + reporte_dos.length * 10;
      ...
      // Cliclo
      const nombre_vivero =
        report.nombre_vivero !== null && report.nombre_vivero !== undefined
          ? report.nombre_vivero
          : 'Consolidado';
      doc.circle(10, 40 + coordendas, 2, 'FD'); // Circulo x vivero
      doc.setFont('Arial', 'bold'); // establece la fuente en Arial
      doc.text(
        dayjs(report.fecha_baja).format('DD/MM/YYYY'),
        14,
        41 + coordendas
      );
      doc.text(
        nombre_vivero,
        doc.internal.pageSize.width / 2 - 21,
        41 + coordendas
      );
      doc.text(
        report.nro_baja_por_tipo.toString(),
      ...
      doc.line(
        ...
        50 + coordendas
      ); // Linea central de tabla
      // Tabla
      autoTable(doc, {
        theme: 'plain',
        columns: [
          { header: 'Bien implicado', dataKey: 'nombre_bien' },
          { header: 'Cantidad dada de baja', dataKey: 'cantidad_baja' },
        ],
        body: reporte_dos,
        ...
        if (page !== 1) {
          coordendas =
            reporte_dos.length === 1
              ...
              : reporte_dos.length === 2
              ? coordenada_y + 15
              : coordenada_y + reporte_dos.length * 5;
          coordenada_y =
            reporte_dos.length === 1
              ? 20 + coordendas
              : reporte_dos.length === 2
              ? 65 + coordendas
              : coordendas + reporte_dos.length * 9;
          doc.line(
            5,
            coordenada_y,
            doc.internal.pageSize.width - 5,
  ...
  const generar_historico_traslados: () => void = () => {
    ...
    reporte.forEach((report: any) => {
      ...
      doc.setFont('Arial', 'bold'); // establece la fuente en Arial
      doc.text(
        dayjs(report.fecha_traslado).format('DD/MM/YYYY'),
        14,
        41 + coordendas
      );
      doc.setFont('Arial', 'normal'); // establece la fuente en Arial
      doc.text(report.nombre_bien, 35, 41 + coordendas);
      doc.text(report.cantidad_a_trasladar.toString(), 85, 41 + coordendas, {
        align: 'center',
      });
      doc.text(report.nombre_vivero_origen, 100, 41 + coordendas);
      doc.text(
        report.nro_lote_origen === null
          ? ''
          : report.nro_lote_origen.toString() +
              ' de ' +
              report.agno_lote_origen.toString(),
        130,
        41 + coordendas
      );
      doc.text(report.nombre_vivero_destino, 150, 41 + coordendas);
      doc.text(
        report.nro_lote_destino_MV === null
          ? ''
          : report.nro_lote_destino_MV.toString() +
              ' de ' +
              report.agno_lote_destino_MV.toString(),
        180,
        41 + coordendas
      );
      doc.line(10, 40 + coordendas, 10, 50 + coordendas); // Linea horizontal
  ...
  const generar_historico_levantamiento_cuarentena: () => void = () => {
    ...
    reporte.forEach((report: any) => {
      ...
      doc.setFontSize(11);
      doc.text(
        dayjs(report.fecha_levantamiento).format('DD/MM/YYYY'),
        14,
        41 + coordendas
      );
      doc.setFont('Arial', 'normal'); // establece la fuente en Arial
      ...
      doc.text(report.cantidad_a_levantar.toString(), 125, 41 + coordendas, {
        align: 'center',
      });
      doc.text(
        report.nro_lote.toString() +
          ' de ' +
          report.agno_lote.toString() +
          ' - Etapa ' +
          report.etapa_lote,
        doc.internal.pageSize.width - 60,
        41 + coordendas
  ...
  const generar_historico_ingreso_cuarentena: () => void = () => {
    ...
    reporte.forEach((report: any) => {
      ...
      doc.setFontSize(11);
      doc.text(
        dayjs(report.fecha_cuarentena).format('DD/MM/YYYY'),
        14,
        41 + coordendas
      );
      doc.setFont('Arial', 'normal'); // establece la fuente en Arial
      ...
      doc.text(report.cantidad_cuarentena.toString(), 140, 41 + coordendas, {
        align: 'center',
      });
  ...
  const generar_historico_cambios_etapa: () => void = () => {
    ...
    reporte.forEach((report: any) => {
      ...
      doc.setFontSize(11);
      doc.text(
        dayjs(report.fecha_cambio).format('DD/MM/YYYY'),
        14,
        41 + coordendas
      );
      doc.setFont('Arial', 'normal'); // establece la fuente en Arial
      ...
      doc.text(report.etapa_destino, 162, 41 + coordendas, {
    ...
    reporte.forEach((report: any) => {
      const nombre_vivero =
        report.nombre_vivero !== null && report.nombre_vivero !== undefined
          ? report.nombre_vivero
          : 'Consolidado';
      const reporte_dos = jso_object_detalle(
        report.bienes_distribuidos,
        nombre_vivero
      );
      // Cliclo
      coordenada_y =
      ...
      doc.setFont('Arial', 'bold'); // establece la fuente en Arial
      doc.text(
        dayjs(report.fecha_distribucion).format('DD/MM/YYYY'),
        14,
        41 + coordendas
      );
      doc.text(
      ...
      doc.text(
        report.numero_despacho.toString(),
        doc.internal.pageSize.width - 50,
        41 + coordendas
      );
      doc.setFont('Arial', 'normal'); // establece la fuente en Arial
        ...
        if (page !== 1) {
          ...
          coordenada_y =
            reporte_dos.length === 1
              ...
              : reporte_dos.length === 2
              ? 67 + coordendas
              : coordendas + reporte_dos.length * 11;
          doc.line(
            5,
            coordenada_y,
            doc.internal.pageSize.width - 5,
  ...
  const generar_historico_siembras: () => void = () => {
    ...
    reporte.forEach((report: any) => {
      ...
      doc.setFont('Arial', 'normal'); // establece la fuente en Arial
      doc.text(
        dayjs(report.fecha_siembra).format('DD/MM/YYYY'),
        14,
        41 + coordendas
      );
      ...
      doc.text(report.nombre_bien, 80, 41 + coordendas);
      doc.text(
        report.distancia_entre_semillas.toString(),
        147,
        41 + coordendas
      );
      ...
      autoTable(doc, {
        theme: 'plain',
        columns: [
          { header: 'Bienes consumidos', dataKey: 'nombre_bien' },
  ...
  const jso_object_detalle: (array: any, nombre_vivero: string) => any = (
    array: any,
          ...
          >
            <Grid item container spacing={2}>
              <Grid item xs={12} sm={6}>
                ...
                {error_reporte && (
                  <FormHelperText error>{mensaje_error}</FormHelperText>
                )}
              </Grid>
              <Grid item xs={12} sm={6}>
              ...
              <Grid item xs={12} sm={5}>
                <Stack direction="row" justifyContent="flex-start" spacing={2}>
                  <Button
                    ...
                    disabled={
                      seleccion_vivero === '' && seleccion_reporte === 'EL'
                    }
                  >
                    Buscar planta
                  </Button>
