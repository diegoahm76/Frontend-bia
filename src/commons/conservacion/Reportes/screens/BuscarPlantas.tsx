...
const BuscarPlantas = (props: IProps) => {
  ...
  useEffect(() => {
    ...
    if (props.reporte === 'EL') {
      set_tamaño_col(3);
      dispatch(obtiene_plantas_por_vivero(props.vivero)).then((response: any) => {
  ...
  const buscar_bien = (): void => {
    ...
    if(props.reporte === 'EL'){
      ...
      if (año_lote !== "" && props.reporte === 'EL')
        data_filter = [...data_filter.filter((da: any) => da.agno_lote.toString().toLowerCase().includes(año_lote))];
      if (numero_lote !== "" && props.reporte === 'EL')
        data_filter = [...data_filter.filter((da: any) => da.nro_lote.toString().toLowerCase().includes(numero_lote))];
    }
          ...
          >
            ...
            >
              <Grid container spacing={2}>
                ...
                {props.reporte === 'EL' && <Grid item xs={12} sm={tamaño_col}>
                  <TextField
                    label="Año lote"
                    helperText=" "
                    size="small"
