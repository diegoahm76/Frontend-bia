...
import { set_persons, set_changing_person } from '../store/slice/produccionSlice';
import { useAppDispatch, useAppSelector } from '../../../../hooks';
import { get_person_document_service, get_person_id_service, get_persons_service } from '../store/thunks/produccionThunks';
...
const PersonaCambia = ({
    ...
    const { changing_person, current_stage_change, persons } = useAppSelector((state) => state.produccion);
    ...
    useEffect(() => {
        reset_persona(changing_person)
    }, [changing_person]);

    useEffect(() => {
        if (current_stage_change.id_persona_cambia !== changing_person.id_persona) { 
            void dispatch(get_person_id_service(Number(current_stage_change.id_persona_cambia) ?? 0)) 
        }
    ...
    const get_personas: any = (async () => {
        ...
        void dispatch(get_persons_service(type, document, primer_nombre, primer_apellido, razon_social, comercial_name))
    })
        ...
        <>
            ...
            >
                <BuscarModelo
                    ...
                    set_current_model={set_changing_person}
                    row_id={"id_persona"}
