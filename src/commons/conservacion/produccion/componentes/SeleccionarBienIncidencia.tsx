...
const SeleccionarBienPreparacion = () => {
  ...
  const { bienes, current_nursery, current_bien, preparacion_bienes } =
  ...
  const get_bienes: any = async () => {
    ...
    if (id_vivero !== null && id_vivero !== undefined) {
      ...
      const tipo = get_values_bien('tipo_bien') ?? 'IN';
      void dispatch(
        get_bienes_incidencia_service(id_vivero, codigo_bien, nombre, tipo)
      );
    }
  };
  ...
  const on_submit_preparacion = (data: IObjPreparacionBienes): void => {
    if (current_bien.id_bien !== null) {
      if (get_values_bien('codigo_bien') === current_bien.codigo_bien) {
        const bien: IObjPreparacionBienes | undefined = aux_insumos.find((p) =>
          current_bien.id_mezcla === null
            ? p.id_bien === current_bien.id_bien
            : p.id_mezcla === current_bien.id_mezcla
        );

        if (
          (data.cantidad_a_consumir ?? 0) <=
          (current_bien.saldo_disponible ?? 0)
        ) {
          const new_bien: IObjPreparacionBienes = {
            id_consumo_insidenciaMV: data.id_consumo_insidenciaMV ?? null,
            id_bien: current_bien.id_bien ?? null,
            id_mezcla: current_bien.id_mezcla ?? null,
  ...
  const edit_bien_preparacion = (item: IObjPreparacionBienes): void => {
    set_action('editar');
    const bien: IObjBienes | undefined = bienes.find((p: IObjBienes) =>
      item.id_mezcla === null
        ? p.id_bien === item.id_bien
        : p.id_mezcla === item.id_mezcla
    );
    ...
    if (bien !== undefined) {
      const restante =
        (bien.saldo_disponible ?? 0) + (item?.cantidad_a_consumir ?? 0);
      if (item.id_consumo_insidenciaMV !== null) {
