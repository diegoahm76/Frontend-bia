...
import { type IObjSolicitudVivero, type IObjNursery } from '../../solicitudMaterial/interfaces/solicitudVivero';
...
import { get_bienes_despacho, get_num_despacho, get_person_id_service as get_person_id_despacho, annul_despacho_service, crear_despacho, editar_despacho, closed_solicitud_service } from '../store/thunks/distribucionThunks';
...
const DespachoScreen = () => {
  ...
  const { control: control_solicitud_aprobada, handleSubmit: handle_submit_solicitud, reset: reset_solicitud, getValues: get_values_solicitud } = useForm<IObjSolicitudVivero>();
  ...
  const on_submit_closed = (data: IObjSolicitudVivero): void => {
    const data_closed = {
      observacion_cierre_no_dispo_viveros: data.observacion_cierre_no_dispo_viveros,
    ...
    if (current_solicitud.id_solicitud_vivero !== null && current_solicitud.id_solicitud_vivero !== undefined) {
      void dispatch(closed_solicitud_service(current_solicitud.id_solicitud_vivero, data_closed));
    }
  };
    ...
    >
      ...
      >
        ...
        <Grid item xs={12} md={4}>
          <AnularEliminar
            action={current_solicitud.solicitud_abierta === true ? "Cerrar por no disponibilidad" : "Detalle del cierre"}
