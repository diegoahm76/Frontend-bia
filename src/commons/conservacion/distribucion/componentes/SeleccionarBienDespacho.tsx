...
import { type IObjBienDespacho, type IObjBien } from "../interfaces/distribucion";
import { set_current_bien, set_bienes_despacho, set_bienes, initial_state_current_bien } from '../store/slice/distribucionSlice';
import { useAppDispatch, useAppSelector } from '../../../../hooks';
import { useEffect, useState } from 'react';
import { control_error, get_bien_code_service, get_bienes_service } from '../store/thunks/distribucionThunks';
...
const SeleccionarBienDespacho = () => {
    ...
    const { control: control_despacho, handleSubmit:handle_submit_despacho, reset: reset_despacho } = useForm<IObjBienDespacho>();
    ...
    const { origin_nursery, current_bien, bienes, current_despacho, bienes_despacho, bien_selected } = useAppSelector((state) => state.distribucion);
    ...
    const get_bienes: any = (async () => {
        ...
        if (id_vivero !== null && id_vivero !== undefined) {
            ...
            void dispatch(get_bienes_service(id_vivero, codigo_bien, nombre, agno, nro));
        }
    })
    ...
    const search_bien: any = (async () => {
        try {
            ...
            if (id_vivero !== null && id_vivero !== undefined) {
                ...
                const data = await dispatch(get_bien_code_service(id_vivero, bien_selected.codigo_bien??"", bien_selected.nombre_bien??"", bien_selected.cod_tipo_elemento_vivero??""));
                set_bienes_aux(data)
            }
          } catch (error) {
            console.error(error);
    ...
    const on_submit_despacho = (data: IObjBienDespacho): void => {   
        if(current_bien.id_bien !== null){
            if(get_values_bien("codigo_bien") === current_bien.codigo_bien){
                const bien: IObjBienDespacho | undefined = aux_insumos.find((p) => (bien_selected.cod_tipo_elemento_vivero === "IN")?p.id_bien === current_bien.id_bien : p.id_bien === current_bien.id_bien && p.nro_lote === current_bien.nro_lote )
                let asignada = 0
                ...
                {  
                    ...
                    {  
                        ...
                        if (bien === undefined) {
                                ...
                                const restante = (current_bien.cantidad_disponible ?? 0) - (new_bien.cantidad_despachada ?? 0) 
                                dispatch(set_current_bien({...current_bien, cantidad_disponible: restante}))
                                reset_despacho({id_bien: current_bien?.id_bien, cantidad_despachada: null, observacion_del_despacho: null});
    ...
    const delete_bien_despacho = (item: IObjBienDespacho): void => {
        ...
        if(item.cod_tipo_elemento_vivero === "IN"){
            ...
            bien =bienes.find((p: IObjBien) => p.id_bien === item.id_bien && p.nro_lote === item.nro_lote)
        }
        console.log(bien)
        ...
        reset_despacho({id_bien: bien?.id_bien, cantidad_despachada: null, observacion_del_despacho: null});
        const aux_items: IObjBienDespacho[] = []
        aux_insumos.forEach((option) => {
            if(item.cod_tipo_elemento_vivero === "IN"){
                if (option.id_bien !== item.id_bien) {
                ...
                if (!(option.id_bien === item.id_bien && option.nro_lote === item.nro_lote)) {
                    aux_items.push(option)
                }
            }
        })
        ...
        <>
            ...
            >
                <BuscarModelo
                    ...
                    form_inputs={ bien_selected.cod_tipo_elemento_vivero === "MV"?
                        [
                        {
                            datum_type: "title",
                    ...
                    form_inputs_list={[
                        {
                            ...
                            default_value: "",
                            rules: { required_rule: { rule: true, message: "Debe ingresar cantidad" }, min_rule: { rule: 0.01, message: "La cantidad debe ser mayor a 0" }, max_rule: { rule: current_bien.cantidad_disponible, message: 'La cqantidad no debe ser mayor que '+ String(current_bien.cantidad_disponible) }},
