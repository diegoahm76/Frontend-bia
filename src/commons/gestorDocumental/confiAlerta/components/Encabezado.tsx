...
export const Encabezado: React.FC = () => {
    ...
    if (name.startsWith('preguntas')) {
      ...
      if (match) {
        ...
        setFormData((prevData) => {
          const newFormData = { ...prevData };
          if (opIndex !== null) {
            (newFormData.preguntas[qIndex] as any).opciones_rta[opIndex][
              field
            ] = value;
          } else {
  ...
  const addNewOption = (preguntaIndex: number, tempOption: string) => {
    ...
    const existeOpcion = formData.preguntas[preguntaIndex]?.opciones_rta?.some(
      (opcion: { opcion_rta: string }) => opcion.opcion_rta === tempOption
    );
    ...
    setFormData((prevData: any) => {
      const newFormData = { ...prevData };
      newFormData.preguntas[preguntaIndex].opciones_rta.push({
        opcion_rta: tempOption,
      });
      return newFormData;
    });
  ...
  const startEditQuestion = (id: string) => {
    const questionToEdit = tempQuestions.find(
      (q) => q.redaccion_pregunta === id
    );
    if (questionToEdit) {
      setFormData({
        ...formData,
      ...
      setGridRows(
        questionToEdit.opciones_rta.map((opt: { opcion_rta: any }) => ({
          opciones: opt.opcion_rta,
        }))
      );
        ...
        <>
          <IconButton
            ...
            onClick={() => {
              ...
              const opciones =
                tempQuestions.find((q) => q.redaccion_pregunta === pregunta)
                  ?.opciones_rta || [];
  ...
  const handleDeleteQuestion = (id: any) => {
    const updatedTempQuestions = tempQuestions.filter(
      (question) => question.redaccion_pregunta !== id
    );
  ...
  const addNewQuestion = () => {
    ...
    if (editMode) {
      ...
      setQuestionGridRows([
        ...questionGridRows,
        {
          id: newQuestion.redaccion_pregunta,
          pregunta: newQuestion.redaccion_pregunta,
        },
      ]);
    }
    // else {
    ...
    const existingQuestion = tempQuestions.find(
      (q) => q.redaccion_pregunta === newQuestion.redaccion_pregunta
    );
    if (existingQuestion) {
      control_error(
        'Ya existe una pregunta con el mismo texto. Por favor ingrese una pregunta diferente.'
      );
      return;
    }
    setTempQuestions([...tempQuestions, newQuestion]);
  ...
  useEffect(() => {
    const fetchEncuestaDetalle = async () => {
      if (selectedEncuestaId) {
        // Solo haz la llamada si selectedEncuestaId tiene un valor válido
        try {
          const res = await api.get(
            `/gestor/encuestas/encabezado_encuesta/get/detalle/${selectedEncuestaId}/`
          );
          if (res.data.success) {
            const { preguntas, nombre_encuesta, descripcion, activo } =
              res.data.data;
            setItemYaUsado(res.data.data.item_ya_usado);
            // Prepara los datos para ser mostrados en el DataGrid
            const newGridRows = (preguntas || []).map(
              (pregunta: { redaccion_pregunta: any }) => ({
  ...
  const handleSubmit = async (event: { preventDefault: () => void }) => {
    ...
    try {
      if (selectedEncuestaId) {
        // Si hay un selectedEncuestaId, actualizamos
        const response = await api.put(
          `/gestor/encuestas/encabezado_encuesta/update/${selectedEncuestaId}/`,
          {
            ...formData,
        ...
        const response = await api.post(
          '/gestor/encuestas/encabezado_encuesta/create/',
          {
            ...formData,
            preguntas: tempQuestions,
          }
  ...
  useEffect(() => {
    if (itemYaUsado) {
      control_warning(
        'El ítem ha sido usado, no se podra actualizar ni eliminar '
      );
    }
  }, [itemYaUsado]);
    ...
    if (selectedEncuestaId) {
      try {
        const res = await api.delete(
          `/gestor/encuestas/encabezado_encuesta/delete/${selectedEncuestaId}/`
        );
        if (res.data.success) {
  ...
  const handleUpdateOption = (oldOption: string, newOption: string) => {
    setGridRows((prevRows) => {
      return prevRows.map((row) =>
        row.opciones === oldOption ? { ...row, opciones: newOption } : row
      );
    });
    setFormData((prevData) => {
      const newFormData = { ...prevData };
      newFormData.preguntas.forEach(
        (pregunta: {
          opciones_rta: { opcion_rta: string; id_opcion_rta?: number }[];
        }) => {
          pregunta.opciones_rta = pregunta.opciones_rta.map(
            (opcion) =>
              opcion.opcion_rta === oldOption
                ? { ...opcion, opcion_rta: newOption }
                : opcion // Mantener el id_opcion_rta
          );
        }
      );
      return newFormData;
    ...
    <>
      <>
        {showContent || selectedEncuestaId ? (
          ...
          <>
            <Grid container spacing={2} m={2} p={2} sx={miEstilo}>
            ...
            <Grid container spacing={2} m={2} p={2} sx={miEstilo}>
              ...
              <Dialog
                open={isVerModalAbierto}
                onClose={cerrarVerModal}
                maxWidth="xl"
              >
            ...
            <Grid container spacing={2} m={2} p={2} sx={miEstilo}>
              <Grid item xs={12} sm={1.4}>
                <Button
                  ...
                  onClick={() => {
                    navigate('/app/gestor_documental/encuesta_datos/datos');
                  }}
                  fullWidth
                  variant="outlined"
                >
            ...
            <Buscar
              ...
              setSelectedEncuestaId={setSelectedEncuestaId}
              is_modal_active={is_buscar}
