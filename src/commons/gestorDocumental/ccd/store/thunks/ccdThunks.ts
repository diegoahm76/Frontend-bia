...
const notification_error = async (message = 'Algo pasó, intente de nuevo', text = '') => await Swal.mixin({
    position: 'center',
    icon: 'error',
    title: message,
    text,
...
export const to_finished_ccds_service = (set_flag_btn_finish: (arg0: boolean) => void) => {
    return async (dispatch: Dispatch<any>, getState:any ): Promise<AxiosResponse | AxiosError> => {        
        const { ccd_current } = getState().ccd;
        try {
            // eslint-disable-next-line @typescript-eslint/restrict-template-expressions
            const { data } = await api.put(`gestor/ccd/finish/${ccd_current.id_ccd}/?confirm=false`);
            dispatch(get_classification_ccds_service());
            control_success(data.detail);
            ...
            if (error.response.data.delete === true) {
                void Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        // eslint-disable-next-line @typescript-eslint/restrict-template-expressions
                        api.put(`gestor/ccd/finish/${ccd_current.id_ccd}/?confirm=true`)
                            .then((response) => {
                ...
                // eslint-disable-next-line @typescript-eslint/restrict-template-expressions
                void notification_error(error.response.data.detail, `Estas son las faltanes: ${error.response.data.data.map((item: any) => item).join(', ')}`);
            }
            return error as AxiosError;
        };
    };
...
export const update_ccds_service = (ccd: { nombre: any; version: any; }) => {
    return async (dispatch: Dispatch<any>, getState:any ): Promise<AxiosResponse | AxiosError> => {        
        const { ccd_current } = getState().ccd;
        try {
            // eslint-disable-next-line @typescript-eslint/restrict-template-expressions
            const { data } = await api.patch(`gestor/ccd/update/${ccd_current.id_ccd}/`, ccd);
            dispatch(get_ccd_current({ ...ccd_current, nombre: ccd.nombre, version: ccd.version }));
            control_success(data.detail);
            return data;
        } catch (error: any) {
            control_error(error.response.data.detail);
