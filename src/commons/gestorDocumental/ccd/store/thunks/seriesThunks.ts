...
export const get_series_service = () => {
    return async (dispatch: Dispatch<any>, getState:any ): Promise<AxiosResponse | AxiosError> => {    
        ...
        try {
            ...
            const { data } = await api.get(`gestor/ccd/series/get/${ccd_current.id_ccd}/`);
...
export const create_series_service = (newSeries: ISeriesObject[], clean: () => void) => {
    return async (dispatch: Dispatch<any>, getState:any ): Promise<AxiosResponse | AxiosError> => {    
        const { ccd_current } = getState().ccd;
        ...
        try {
            // eslint-disable-next-line @typescript-eslint/restrict-template-expressions
            const { data } = await api.put(`gestor/ccd/series/update/${ccd_current.id_ccd}/`, newSeries);
