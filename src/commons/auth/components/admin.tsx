...
import {
  ...
  consultar_datos_persona,
  consultar_vinculacion_persona,
...
export const AdministracionPersonasScreenNatural: React.FC<Props> = ({
  ...
  const [clase_tercero_persona, set_clase_tercero_persona] = useState<
    ClaseTercero[]
  >([]);
  const [datos_vinculacion, set_datos_vinculacion] =
    useState<DatosVinculacionCormacarena>();
  ...
  const [historico_direcciones, set_historico_direcciones] =
    useState<boolean>(false);
  const [historico, set_historico] = useState<boolean>(false);
  const [historico_autorizacion, set_historico_autorizacion] =
    useState<boolean>(false);
  const [loading_natural, set_loading_natural] = useState<boolean>(false);
  const [datos_historico_autorizacion, set_datos_historico_autorizacion] =
    useState<InfoPersona>({
      id: 0,
      id_persona: 0,
      tipo_persona: '',
      tipo_documento: '',
  ...
  const [datos_historico_email, set_datos_historico_email] =
    useState<InfoPersona>({
      id: 0,
      id_persona: 0,
      tipo_persona: '',
  ...
  const [datos_historico_direcciones, set_datos_historico_direcciones] =
    useState<InfoPersona>({
      id: 0,
      id_persona: 0,
      tipo_persona: '',
    ...
    try {
      ...
      set_data_register({
        ...
        direccion_notificacion_referencia:
          response?.direccion_notificacion_referencia,
        // Autorización
  ...
  const on_submit_update_natural = handle_submit(async (data) => {
    try {
      if (
        data.datos_clasificacion_persona === undefined ||
        data.datos_clasificacion_persona.length === 0
      ) {
      ...
      if (data.email === data.email_empresarial) {
        control_error(
          'E-mail empresarial debe ser diferente de E-mail personal'
        );
        return;
      }
      ...
      await editar_persona_natural(
        data_all?.id_persona,
        datos_persona as DataNaturaUpdate
      );
  ...
  return (
    <>
          ...
          {datos_persona != null && (
            <Grid container spacing={2}>
              ...
              <>
                ...
                <Grid item xs={12}>
                  <Typography variant="subtitle1" fontWeight="bold">
                    Dirección de notificación Nacional
                  </Typography>
                </Grid>
                <>
                  <Grid item xs={12} sm={6} md={4}>
              ...
              <>
                ...
                <Grid item xs={12} sm={3}>
                  <TextField
                    ...
                    multiline
                    value={
                      datos_vinculacion?.observaciones_vinculacion_cargo_actual
                    }
                  />
                </Grid>
                <Grid item xs={12}>
      ...
      <DialogAutorizaDatos
        ...
        data_autorizacion={{
          acepta_autorizacion_email: data_register.acepta_notificacion_email,
          acepta_autorizacion_sms: data_register.acepta_notificacion_sms
        }}
