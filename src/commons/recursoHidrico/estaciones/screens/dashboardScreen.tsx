...
export const DashboardScreen: React.FC = () => {
  ...
  const formatdataforchart = (data: Datos[]) => {
    ...
    opc_variables.forEach((item) => {
      item.value === 1
        ? data_aux.push({ ...item, data: data_temperatura })
        : item.value === 2
        ? data_aux.push({ ...item, data: data_humedad })
        : item.value === 3
        ? data_aux.push({ ...item, data: data_luminosidad })
        : item.value === 4
        ? data_aux.push({ ...item, data: data_velocidad_agua })
        : item.value === 5
        ? data_aux.push({ ...item, data: data_nivel_agua })
        : item.value === 6
        ? data_aux.push({ ...item, data: data_velocidad_viento })
        : item.value === 7
        ? data_aux.push({ ...item, data: data_direccion_viento })
        : item.value === 8
  ...
  const formatdataforchart_migracion = (data: DatosMigracionGraficas[]) => {
    ...
    opc_variables_migracion.forEach((item) => {
      item.value === 1
        ...
        ? data_aux.push({ ...item, data: data_humedad })
        : item.value === 3
        ? data_aux.push({ ...item, data: data_pto_rocio })
        : item.value === 4
        ? data_aux.push({ ...item, data: data_presion_rel })
        : item.value === 5
        ? data_aux.push({ ...item, data: data_nivel_agua })
        : item.value === 6
        ? data_aux.push({ ...item, data: data_precipitacion })
        : item.value === 7
        ? data_aux.push({ ...item, data: data_data_presion_abs })
        : item.value === 8
        ? data_aux.push({ ...item, data: data_velocidad_rio })
