...
export const AnaliticaScreen: React.FC = () => {
  ...
  const [opc_variables, set_opc_variables] = useState<Variables[]>([
    ...
    {
      title: 'Precipitación',
      chart_id: 'precipitacion',
      data: [],
      data_comparacion: [],
      value: 8,
    },
    {
      title: 'Presión barometrica',
      chart_id: 'presion',
      data: [],
      data_comparacion: [],
      value: 9,
  ...
  const [opc_variables_migracion, set_opc_variables_migracion] = useState<
    Variables[]
  >([
    {
      title: 'Temperatura',
    ...
    },
    {
      ...
      chart_id: 'punto-rocio',
      data: [],
      data_comparacion: [],
      value: 3,
    },
    {
      title: 'Presion atm abs',
      chart_id: 'presion-atm-abs',
      data: [],
      data_comparacion: [],
      value: 4,
    },
    ...
    {
      title: 'Presion atm rel',
      chart_id: 'presion-atm-rel',
      data: [],
      data_comparacion: [],
      value: 7,
    },
    {
      title: 'Velocidad del rio',
      chart_id: 'velocidad-rio',
      data: [],
      data_comparacion: [],
      value: 8,
    },
  ...
  const [variable_selected, set_variable_selected] = useState<Variables>({
    title: 'Nivel de Agua',
    chart_id: 'Nivel de agua',
    data: [],
    data_comparacion: [],
    value: 5,
  });
  const [variable_selected_migracion, set_variable_selected_migracion] =
    useState<Variables>({
      title: 'Nivel de Agua',
      chart_id: 'Nivel de agua',
      data: [],
  ...
  const [start_date_comparacion, set_start_date_comparacion] =
    useState<Date | null>(new Date());
  const [end_date_comparacion, set_end_date_comparacion] =
    useState<Date | null>(new Date());
  ...
  const get_datos_estaciones_migracion = async (): Promise<any> => {
    try {
      ...
      if ('data' in data && 'data' in data_comparacion) {
        ...
        const formatted_data_comparacion = data_comparacion.data.map(
          (item: any) => {
            ...
            Object.keys(formatted_item_compatacion).forEach((key) => {
              if (!isNaN(Number(formatted_item_compatacion[key]))) {
                formatted_item_compatacion[key] = Number(
                  formatted_item_compatacion[key]
                );
              }
            });
        ...
        formatdataforchart_migracion(
          formatted_data,
          formatted_data_comparacion
        );
        set_loading_busqueda(false);
      } else {
        control_error('No se encontraron datos');
      ...
      if (temp_error.response?.status === 404) {
        set_loading_busqueda(false);
        control_error(
          'No se encontraron datos para esta estación en esta fecha'
        );
      } else {
        // Otro error, mostrar mensaje de error genérico
        set_loading_busqueda(false);
  ...
  useEffect(() => {
    if (variable_selected !== undefined) {
      const update_variable: Variables =
        opc_variables.find(
          (variable) => variable.value === variable_selected.value
        ) ?? variable_selected;
  ...
  useEffect(() => {
    if (variable_selected_migracion !== undefined) {
      const update_variable: Variables =
        opc_variables_migracion.find(
          (variable) => variable.value === variable_selected_migracion.value
        ) ?? variable_selected_migracion;
      set_variable_selected(update_variable);
      set_variable_selected_migracion(update_variable);
    }
  ...
  const formatdataforchart = (data: Datos[], data_comparacion: Datos[]) => {
    ...
    data.forEach((item) => {
      data_temperatura.push([
        new Date(item.fecha_registro).getTime(),
        Number(item.temperatura_ambiente),
      ]);
      data_humedad.push([
        new Date(item.fecha_registro).getTime(),
        Number(item.humedad_ambiente),
      ]);
      data_luminosidad.push([
        new Date(item.fecha_registro).getTime(),
        Number(item.luminosidad),
      ]);
      data_nivel_agua.push([
        new Date(item.fecha_registro).getTime(),
        Number(item.nivel_agua),
      ]);
      data_velocidad_agua.push([
        new Date(item.fecha_registro).getTime(),
        Number(item.velocidad_agua),
      ]);
      data_velocidad_viento.push([
        new Date(item.fecha_registro).getTime(),
        Number(item.velocidad_viento),
      ]);
      data_direccion_viento.push([
        new Date(item.fecha_registro).getTime(),
        Number(item.direccion_viento),
      ]);
      data_precipitacion.push([
        new Date(item.fecha_registro).getTime(),
        Number(item.precipitacion),
      ]);
      data_presion.push([
        new Date(item.fecha_registro).getTime(),
        Number(item.presion_barometrica),
      ]);
    });
    ...
    data_comparacion.forEach((item) => {
      data_temperatura_comparacion.push([
        new Date(item.fecha_registro).getTime(),
        Number(item.temperatura_ambiente),
      ]);
      data_humedad_comparacion.push([
        new Date(item.fecha_registro).getTime(),
        Number(item.humedad_ambiente),
      ]);
      data_luminosidad_comparacion.push([
        new Date(item.fecha_registro).getTime(),
        Number(item.luminosidad),
      ]);
      data_nivel_agua_comparacion.push([
        new Date(item.fecha_registro).getTime(),
        Number(item.nivel_agua),
      ]);
      data_velocidad_agua_comparacion.push([
        new Date(item.fecha_registro).getTime(),
        Number(item.velocidad_agua),
      ]);
      data_velocidad_viento_comparacion.push([
        new Date(item.fecha_registro).getTime(),
        Number(item.velocidad_viento),
      ]);
      data_direccion_viento_comparacion.push([
        new Date(item.fecha_registro).getTime(),
        Number(item.direccion_viento),
      ]);
      data_precipitacion_comparacion.push([
        new Date(item.fecha_registro).getTime(),
        Number(item.precipitacion),
      ]);
      data_presion_comparacion.push([
        new Date(item.fecha_registro).getTime(),
        Number(item.presion_barometrica),
      ]);
    });
    ...
    opc_variables.forEach((item) => {
      item.value === 1
        ? data_aux.push({
            ...
            data_comparacion: data_temperatura_comparacion,
          })
        : item.value === 2
        ? data_aux.push({
            ...item,
            data: data_humedad,
            data_comparacion: data_humedad_comparacion,
          })
        : item.value === 3
        ? data_aux.push({
            ...item,
            data: data_luminosidad,
            data_comparacion: data_luminosidad_comparacion,
          })
        : item.value === 4
        ? data_aux.push({
            ...item,
            data: data_velocidad_agua,
            data_comparacion: data_velocidad_agua_comparacion,
          })
        : item.value === 5
        ? data_aux.push({
            ...item,
            data: data_nivel_agua,
            data_comparacion: data_nivel_agua_comparacion,
          })
        : item.value === 6
        ? data_aux.push({ ...item, data: data_velocidad_viento })
        : item.value === 7
        ? data_aux.push({
            ...item,
            data: data_direccion_viento,
            data_comparacion: data_direccion_viento_comparacion,
          })
        : item.value === 8
        ? data_aux.push({
            ...item,
            data: data_precipitacion,
            data_comparacion: data_precipitacion_comparacion,
          })
        : data_aux.push({
            ...item,
            data: data_presion,
            data_comparacion: data_presion_comparacion,
          });
    });
  ...
  // eslint-disable-next-line @typescript-eslint/explicit-function-return-type
  const formatdataforchart_migracion = (
    data: DatosMigracionGraficas[],
    data_comparacion: DatosMigracionGraficas[]
  ) => {
    const data_temperatura: number[][] = [];
    const data_humedad: number[][] = [];
    ...
    data.forEach((item) => {
      ...
      data_humedad.push([
        new Date(item.fecha).getTime(),
        Number(item.humedad_relativa),
      ]);
      data_pto_rocio.push([
        new Date(item.fecha).getTime(),
        Number(item.punto_de_rocio),
      ]);
      data_data_presion_abs.push([
        new Date(item.fecha).getTime(),
        Number(item.presion_atm_abs),
      ]);
      data_presion_rel.push([
        new Date(item.fecha).getTime(),
        Number(item.presion_atm_rel),
      ]);
      data_precipitacion.push([
        new Date(item.fecha).getTime(),
        Number(item.precipitacion),
      ...
      data_velocidad_rio.push([
        new Date(item.fecha).getTime(),
        Number(item.velocidad_rio),
      ]);
    ...
    data_comparacion.forEach((item) => {
      ...
      data_pto_rocio_comparacion.push([
        new Date(item.fecha).getTime(),
        Number(item.punto_de_rocio),
      ]);
      data_data_presion_abs_comparacion.push([
        new Date(item.fecha).getTime(),
        Number(item.presion_atm_abs),
      ]);
      data_presion_rel_comparacion.push([
        new Date(item.fecha).getTime(),
        Number(item.presion_atm_rel),
      ]);
      data_precipitacion_comparacion.push([
      ...
      data_velocidad_rio_comparacion.push([
        new Date(item.fecha).getTime(),
        Number(item.velocidad_rio),
      ]);
      data_caudal_comparacion.push([
        new Date(item.fecha).getTime(),
        Number(item.caudal),
      ]);
    });
    ...
    opc_variables_migracion.forEach((item) => {
      item.value === 1
        ...
        ? data_aux.push({
            ...item,
            data: data_pto_rocio,
            data_comparacion: data_pto_rocio_comparacion,
          })
        : item.value === 4
        ? data_aux.push({
            ...item,
            data: data_data_presion_abs,
            data_comparacion: data_data_presion_abs_comparacion,
          })
        : item.value === 5
        ? data_aux.push({
            ...item,
        ...
        ? data_aux.push({
            ...item,
            data: data_presion_rel,
            data_comparacion: data_presion_rel_comparacion,
          })
        : item.value === 8
        ? data_aux.push({
            ...item,
            data: data_velocidad_rio,
            data_comparacion: data_velocidad_rio_comparacion,
          })
        : data_aux.push({
            ...item,
            data: data_caudal,
            data_comparacion: data_caudal_comparacion,
          });
    });
                ...
                <>
                  <Grid container spacing={2}>
                    ...
                    <Grid item xs={12} sm={6}>
                      <Grid border={0.5}>
                        <ChartData
                          data={variable_selected.data}
                          chart_id={variable_selected.chart_id}
                        />
                      </Grid>
                    </Grid>
                    <Grid item xs={12} sm={6}>
                      <Grid border={0.5} spacing={2}>
                        <ChartData
                          data={variable_selected.data_comparacion}
                          chart_id={variable_selected.chart_id}
                        />
                      </Grid>
                    </Grid>
                ...
                <>
                  <Grid container spacing={2}>
                    ...
                    <Grid item xs={12} sm={6}>
                      <Grid border={0.5}>
                        <ChartData
                          data={variable_selected_migracion.data}
                          chart_id={variable_selected_migracion.chart_id}
                        />
                      </Grid>
                    </Grid>
                    <Grid item xs={12} sm={6}>
                      <Grid border={0.5}>
                        <ChartData
                          data={variable_selected_migracion.data_comparacion}
                          chart_id={variable_selected_migracion.chart_id}
                        />
                      </Grid>
                    </Grid>
