...
export const ReportesScreen: React.FC = () => {
  ...
  const fetch_data_2 = async (): Promise<any> => {
    set_loading(true);
    const fecha = dayjs(fecha_inicial).format("YYYY-MM");
    ...
    if (all_days_have_data) {
      console.log("Hay datos en todos los días únicos del mes.");
    } else {
      console.log("Faltan datos en algunos días únicos del mes.");
      console.log("Días sin datos:");
      for (let day in unique_days) {
        if (unique_days.hasOwnProperty(day) && !unique_days[day]) {
          console.log(day);
        }
      }
    }
  ...
  const handle_download_pdf_2 = async (): Promise<void> => {
    ...
    const title = `CERTIFICADO DE MENSUAL ${selected_station?.label ?? 'Ninguna estación seleccionada'}`;
