...
export const CrearConfiAlertaDialog: React.FC<IProps> = ({
  is_modal_active,
  set_is_modal_active,
  confi_alerta_persona,
}) => {
  ...
  const on_sumbit_alerta: SubmitHandler<FieldValues> = (data): void => {
    ...
    const alerta_existente = conf_alert_person.find(
      (alert: conf_alarma) =>
        alert.nombre_variable_alarma === variable_seleccionada
    );

    if (alerta_existente != null) {
      control_error(
        'La alerta ya existe, por lo tanto edita el mensaje en la editar'
      );
    } else {
  ...
  return (
    <Dialog open={is_modal_active} onClose={handle_close} maxWidth="xs">
      <Box component="form" onSubmit={handleSubmit(on_sumbit_alerta)}>
        <Grid
          container
          spacing={2}
          sx={{
        ...
        >
          <Title title=" Crear Configuración Alerta Estación " />
        </Grid>
        ...
        >
          <Grid item xs={12}>
            <TextField
              ...
              helperText={
                errors.nombre_variable_alarma != null
          ...
          <Grid item xs={12}>
            <TextField
              ...
              helperText={
                errors.mensaje_alarma_maximo != null
                  ? 'Este campo es obligatorio'
                  : ''
              }
            />
          ...
          <Grid item xs={12}>
            <TextField
              ...
              helperText={
                errors.mensaje_alarma_minimo != null
                  ? 'Este campo es obligatorio'
                  : ''
              }
            />
          ...
          <Grid item xs={12}>
            <DialogActions>
              ...
              <Button
                ...
                variant="contained"
                color="success"
                onClick={handleSubmit(on_sumbit_alerta)}
              >
                Guardar
              </Button>
            </DialogActions>
