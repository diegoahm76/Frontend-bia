...
import { consultar_datos_guayuriba_paginado } from '../../requets/Request';
...
export const HistorialDatosGuayuriba: React.FC = () => {
    const [dato, set_dato] = useState<Datos[]>([]);

    const historial_datos = async (): Promise<void> => {
        try {
            const response = await consultar_datos_guayuriba_paginado();
