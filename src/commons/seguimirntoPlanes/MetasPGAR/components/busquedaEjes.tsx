...
import { search_eje_pgar } from '../../SeguimientoPGAR/services/services';


// eslint-disable-next-line @typescript-eslint/naming-convention
export const BusquedaEjes: React.FC = () => {
  ...
  const on_submit_advance = handle_submit(async ({ nombre_objetivo, nombre }) => {
    set_is_search(true);
    try {
      set_rows([]);
      const {
        data: { data },
      } = await search_eje_pgar({
        nombre_objetivo,
        nombre,
      });
  ...
  return (
    <>
      ...
      >
        <Grid item xs={12}>
          <Title title="Búsqueda de Ejes Estratégicos" />
        </Grid>
        <Grid item xs={12}>
          <Divider />
        </Grid>
