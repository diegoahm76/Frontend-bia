...
import { crear_intervalos } from '../../Request/request';
...
export const AgregarIntervalo: React.FC<IProps> = ({
  is_modal_active,
  set_is_modal_active,
  get_datos,
}) => {
  ...
  const on_submit: SubmitHandler<FieldValues> = async (data) => {
    try {
      set_is_loading(true);
      await crear_intervalos(data as IIntervalo);
      set_is_modal_active(false);
