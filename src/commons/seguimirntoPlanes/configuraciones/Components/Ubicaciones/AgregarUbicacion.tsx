...
import { crear_ubicacion } from '../../Request/request';
...
export const AgregarUbicacion: React.FC<IProps> = ({
  is_modal_active,
  set_is_modal_active,
  get_datos,
}) => {
  ...
  const on_submit: SubmitHandler<FieldValues> = async (data) => {
    try {
      set_is_loading(true);
      await crear_ubicacion(data as IUbicacion);
      set_is_modal_active(false);
