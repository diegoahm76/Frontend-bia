...
import { columsPrograma } from './colums';
// eslint-disable-next-line @typescript-eslint/naming-convention
export const ConsultarProgramasPlanes: React.FC = () => {
  const columns_programa: GridColDef[] = [
    ...columsPrograma,
    {
      field: 'cumplio',
      headerName: 'CUMPLIO',
  ...
  const { rows_programas } = useConsultaMetasHook();
  ...
  useEffect(() => {
    const cumplidoTrue = rows_programas.filter(
      (row: { cumplio: boolean }) => row.cumplio === true
    ).length;
    const cumplidoFalse = rows_programas.filter(
