...
import { columsIndicadores } from './colums';
// eslint-disable-next-line @typescript-eslint/naming-convention
export const ConsultarIndicadores: React.FC = () => {
  const columns_indicadores: GridColDef[] = [
    ...columsIndicadores,
    {
      field: 'cumplio',
      headerName: 'CUMPLIO',
  ...
  const { rows_indicadores } = useConsultaMetasHook();
  ...
  useEffect(() => {
    const cumplidoTrue = rows_indicadores.filter(
      (row: { cumplio: boolean }) => row.cumplio === true
    ).length;
    const cumplidoFalse = rows_indicadores.filter(
