...
export const put_anular_solicitud: any = (id_despacho_activo: string, form_data: { justificacion_anulacion: string}) => {
  return async () => {
    try {
      const { data } = await api.put(`/almacen/activos/anular-despacho-autorizado/${id_despacho_activo}/`, form_data);
      return data;
    } catch (error: any) {
      return error as AxiosError;
    }
...
export const put_editar_anexo_despacho: any = (id_despacho_activo: string, form_data: any) => {
  return async () => {
    try {
      const { data } = await api.put(`/almacen/activos/actualizar-anexo-despacho-activos/${id_despacho_activo}/`, form_data);
      return data;
    } catch (error: any) {
      return error as AxiosError;
    }
