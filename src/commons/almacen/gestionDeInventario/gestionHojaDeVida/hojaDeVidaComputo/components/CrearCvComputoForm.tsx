...
import {
  create_cv_computers_service,
  get_marca_service,
} from '../store/thunks/cvComputoThunks';
...
const CrearCvComputoForm = ({
  ...
  const { current_computer, current_cv_computer } = useAppSelector(
    (state) => state.cv
  );
  ...
  useEffect(() => {
    const get_selects_options: any = async () => {
      try {
        ...
        const state_format = text_choise_adapter(state_no_format);
