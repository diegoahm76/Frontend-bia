...
import { interface_busqueda_operario, interface_busqueda_responsable, interface_tipos_documentos, response_busqueda_operario, response_busqueda_responsable, response_tipos_documentos } from '../interfeces/types';
import { get_obtener_operarios, get_obtener_responsables, get_obtener_tipos_documentos } from '../thunks/despacho_solicitudes';
...
const ModalBusquedaFuncionarios: React.FC<props> = ({
  ...
  useEffect(() => {
    if (!busqueda_responsable_obtenida.current && mostrar_modal_busqueda_funcionarios) {
      if (tipo_funcionario === 'responsable') {
  ...
  const consultar_responsable = () => {
    if (tipo_funcionario === 'operario') {
      get_obtener_operarios_fc();
    } else if (tipo_funcionario === 'responsable') {
      get_obtener_responsables_fc();
    }
  }
  ...
  const enviar_funcionario_seleccionado = () => {
    ...
    } else if (tipo_funcionario === 'operario') {
  ...
  }, [funcionario_responsable_temp, funcionario_operario_temp])
    ...
    <>
      <Dialog
        ...
        <DialogContent>
          ...
          >
            <Title title={tipo_funcionario === 'responsable' ?
