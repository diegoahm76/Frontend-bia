...
import { interface_busqueda_operario, interface_busqueda_responsable } from '../interfeces/types';
  ...
  const asignar_funcionario_seleccionado = (newSelectionModel: GridSelectionModel) => {
    if (tipo_funcionario === 'responsable') {
      if (newSelectionModel.length > 0) {
        ...
        set_funcionario_responsable_temp(funcionario);
      }
  ...
  return (
    <>
      ...
      <DataGrid
        ...
        rows={tipo_funcionario === 'responsable' ? (data_funcionarios_responsables ?? []) : (data_funcionarios_operarios ?? [])}
        columns={columns ?? []}
        pageSize={5}
        rowHeight={75}
