...
import { interface_obtener_solicitudes } from '../interfaces/types';
...
interface Props {
  ...
  set_data_resumen_solicitud: React.Dispatch<React.SetStateAction<any>>;
}
    ...
    .then((response: any) => {
      if(Object.keys(response).length !== 0){
        if(response.success){
          ...
          set_data_resumen_solicitud({} as any);
        }
      } else {
