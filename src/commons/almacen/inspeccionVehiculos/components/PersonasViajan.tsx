...
import { get_obtener_tipos_documentos } from '../thunks/inspeccion_vehiculos';
...
interface props {
  ...
  id_solicitud_viaje: number | null;
}
...
const PersonasViajan: FC<props> = ({
  ...
  const handle_agregar_articulo = async() => {
    ...
    if(validacion){
      set_data_personas_viajan([...data_personas_viajan, {
        ...data_persona_seleccionada,
        persona_agregada_inspeccion: true,
        persona_confirma_viaje: true,
        id_solicitud_viaje: id_solicitud_viaje ?? 0,
        observacion: '',
      }]);
