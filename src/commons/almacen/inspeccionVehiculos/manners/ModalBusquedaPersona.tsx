...
import { interface_busqueda_persona_solicita, interface_tipos_documentos, response_busqueda_persona_solicita, response_tipos_documentos } from '../interfaces/types';
import { get_obtener_persona_solicita, get_obtener_tipos_documentos } from '../thunks/inspeccion_vehiculos';
...
interface props {
  ...
  set_data_persona_seleccionada: React.Dispatch<React.SetStateAction<interface_busqueda_persona_solicita>>;
}
...
const ModalBusquedaPersona: React.FC<props> = ({
  ...
  const [data_persona_responsable_temp, set_data_persona_solicita_temp] = useState<interface_busqueda_persona_solicita>(Object);
  // Data de la tabla de personas responsables
  const [data_personas_responsable, set_data_personas_responsable] = useState<interface_busqueda_persona_solicita[]>([]);
    ...
    dispatch(get_obtener_persona_solicita(
      ...
      if (Object.keys(response).length !== 0) {
        if (response.data.length !== 0) {
          set_data_personas_responsable(response.data);
          set_loadding_tabla(false);
        } else {
          set_data_personas_responsable([]);
  ...
  const enviar_persona_solicita_seleccionado = () => {
    if (Object.keys(data_persona_responsable_temp).length !== 0) {
      ...
      set_data_persona_seleccionada(data_persona_responsable_temp);
    ...
    <>
      <Dialog
        ...
        <DialogContent>
          ...
          >
            <Title title='Búsqueda de persona que solicita' />
            <Box
              component="form"
              noValidate
              autoComplete="off"
            ...
            <Grid item container xs={12} sx={{
              ...
              <TablaModalBusquedaPersona
                ...
                data_personas_responsable={data_personas_responsable}
            ...
            <Grid container item xs={12} spacing={1} sx={{ display: 'flex', justifyContent: 'end' }}>
              <Grid item xs={12} md={2} sx={{
                ...
                <Button
                  ...
                  variant='contained'
                  startIcon={<SaveIcon />}
                  disabled={Object.keys(data_persona_responsable_temp).length === 0}
