...
import { interface_busqueda_operario } from '../interfaces/types';

interface CustomColumn extends GridColDef {
  renderCell?: (params: { row: interface_busqueda_operario }) => React.ReactNode;
}
...
const TablaModalBusquedaOperario: React.FC<Props> = ({
  ...
  const asignar_funcionario_responsable = (newSelectionModel: GridSelectionModel) => {
    if (newSelectionModel.length > 0) {
      const vehiculo_seleccionado = data_funcionarios_operarios.find(row => row.id_persona === newSelectionModel[0]);
      const vehiculo_arrendado = vehiculo_seleccionado ?? Object;
      set_funcionario_operario_temp(vehiculo_arrendado);
    }
  }
      ...
      <Grid item xs={12} container
        ...
        <Grid item  >
          <ButtonGroup style={{ margin: 5, }}>
              {download_xls({ nurseries: data_funcionarios_operarios, columns })}
              {download_pdf({
                  nurseries: data_funcionarios_operarios,
                  columns,
...
export default TablaModalBusquedaOperario;