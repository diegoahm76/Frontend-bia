...
import { interface_busqueda_persona_solicita, response_busqueda_persona_solicita, interface_tipos_documentos, response_tipos_documentos } from '../interfaces/types';
import TablaModalBusquedaPersonaSolicita from "../tables/TablaModalBusquedaPersonaSolicita";
import { useAppDispatch } from '../../../../hooks';
import { control_error } from '../../../../helpers';
import ClearIcon from '@mui/icons-material/Clear';
import SaveIcon from '@mui/icons-material/Save';
import { get_obtener_persona_solicita, get_obtener_tipos_documentos } from '../thunks/autorizacion_solicitud_activos';



interface props {
  set_mostrar_modal_persona_solicita: React.Dispatch<React.SetStateAction<boolean>>;
  mostrar_modal_persona_solicita: boolean;
  set_data_persona_solicita: React.Dispatch<React.SetStateAction<interface_busqueda_persona_solicita>>;
}

// eslint-disable-next-line @typescript-eslint/naming-convention
const ModalBusquedaPersonaSolicita: React.FC<props> = ({
  set_mostrar_modal_persona_solicita,
  mostrar_modal_persona_solicita,
  ...
  useEffect(()=>{
    if(!operarios_tp_documentos_obtenidos.current && mostrar_modal_persona_solicita){
  ...
  const enviar_persona_solicita_seleccionado = () => {
    if(Object.keys(data_persona_solicita_temp).length !== 0){
      set_mostrar_modal_persona_solicita(false);
  ...
  return (
    <>
      <Dialog
        open={mostrar_modal_persona_solicita}
        onClose={() => {
          set_mostrar_modal_persona_solicita(false);
        }}
            ...
            <Grid container item xs={12} spacing={1} sx={{display:'flex', justifyContent:'end'}}>
              ...
              <Grid item xs={12} md={2} sx={{
                ...
                <Button
                  ...
                  variant="contained"
                  startIcon={<ClearIcon />}
                  onClick={()=>set_mostrar_modal_persona_solicita(false)}
                >
                  Salir
                </Button>
              </Grid>
...
export default ModalBusquedaPersonaSolicita;