...
interface Props {
  data_personas_solicita: interface_busqueda_persona_solicita[];
...
const TablaModalBusquedaPersona: React.FC<Props> = ({
  ...
  const asignar_funcionario = (newSelectionModel: GridSelectionModel) => {
    if (newSelectionModel.length > 0) {
      const vehiculo_seleccionado = data_personas_solicita.find(row => row.id_persona === newSelectionModel[0]);
      ...
      <Grid item xs={12} container
        ...
        <Grid item  >
          <ButtonGroup style={{ margin: 5, }}>
            {download_xls({ nurseries: data_personas_solicita, columns })}
            {download_pdf({
              nurseries: data_personas_solicita,
              columns,
