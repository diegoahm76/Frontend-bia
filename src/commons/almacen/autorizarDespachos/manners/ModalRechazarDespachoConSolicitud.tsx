...
import { put_rechazar_solicitud } from '../thunks/autorizar_despachos';



interface props {
  set_mostar_modal_rechazar_solicitud: React.Dispatch<React.SetStateAction<boolean>>;
  mostar_modal_rechazar_solicitud: boolean;
...
const ModalRechazarDespachoConSolicitud: React.FC<props> = ({
  set_mostar_modal_rechazar_solicitud,
  mostar_modal_rechazar_solicitud,
  ...
  const rechazar_despacho_con_solicitud = () => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        await dispatch(put_rechazar_solicitud(id_solicitud_activo,{
          justificacion_anulacion: justificacion_rechazo
        }))
          .then((response: any) => {
            if (Object.keys(response).length !== 0) {
              control_success('Solicitud rechazada correctamente');
              get_obtener_despachos_fc();
              set_mostar_modal_rechazar_solicitud(false);
              limpiar_form();
  ...
  return (
    <>
      <Dialog
        open={mostar_modal_rechazar_solicitud}
        onClose={() => {
          set_mostar_modal_rechazar_solicitud(false);
        }}
        ...
        <DialogContent>
          <Grid
            ...
            >
            <Title title={'Justificación del rechazo'} />
            <Box
              component="form"
              noValidate
              autoComplete="off"
            ...
            <Grid container item xs={12} spacing={1} sx={{display:'flex', justifyContent:'end'}}>
              ...
              <Grid item xs={12} md={2}>
                <Button
                  ...
                  variant="contained"
                  startIcon={<ClearIcon />}
                  onClick={()=>set_mostar_modal_rechazar_solicitud(false)}
                >
                  Salir
                </Button>
              </Grid>
