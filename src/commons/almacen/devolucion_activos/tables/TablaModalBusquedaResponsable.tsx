...
import { interface_busqueda_responsable } from '../interfaces/types';
...
const TablaModalBusquedaResponsable: React.FC<Props> = ({
  ...
  const asignar_funcionario_responsable = (newSelectionModel: GridSelectionModel) => {
    if (newSelectionModel.length > 0) {
      const vehiculo_seleccionado = data_funcionarios_responsables.find(row => row.id_persona === newSelectionModel[0]);
      const vehiculo_arrendado = vehiculo_seleccionado ?? Object;
      set_funcionario_responsable_temp(vehiculo_arrendado);
    }
  }
...
export default TablaModalBusquedaResponsable;