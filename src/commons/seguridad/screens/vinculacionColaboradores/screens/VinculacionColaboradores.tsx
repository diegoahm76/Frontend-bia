...
export const VinculacionColaboradoresScreen: React.FC = () => {
    ...
    const [modal_vincular_persona, set_modal_vincular_persona] = useState<boolean>(false);
    ...
    const [persona_vinculacion, set_persona_vinculacion] = useState<any | null>(null);
    const [persona_vinculacion_tkx, set_persona_vinculacion_tkx] = useState<any | null>(null);
    const [cargo_actual, set_cargo_actual] = useState<string>("");
    const [msj_error_cargo_actual, set_msj_error_cargo_actual] = useState<string>("");
    ...
    const [msj_error_fecha_finaliza, set_msj_error_fecha_finaliza] = useState<string>("");
    const [obs_vin_cargo, set_obs_vin_cargo] = useState<string>("");
    const [msj_error_obs_vin_cargo, set_msj_error_obs_vin_cargo] = useState<string>("");
    ...
    useEffect(() => {
        if (persona_vinculacion_tkx !== null && persona_vinculacion_tkx !== undefined) {
            set_cargo_actual(persona_vinculacion_tkx.cargo_actual);
            set_fecha_inicio(dayjs(persona_vinculacion_tkx.fecha_inicio_cargo_actual).format("DD-MM-YYYY"));
            set_fecha_finaliza(dayjs(persona_vinculacion_tkx.fecha_a_finalizar_cargo_actual).format("DD-MM-YYYY"));
            set_obs_vin_cargo(persona_vinculacion_tkx.observaciones_vinculacion_cargo_actual);
            set_unidad_org(persona_vinculacion_tkx.unidad_organizacional_actual);
            set_fecha_asig(dayjs(persona_vinculacion_tkx.fecha_asignacion_unidad).format("DD-MM-YYYY"));
            const formulario_cargo_unidad: any = {
                ...
                fecha_asignacion: dayjs(persona_vinculacion_tkx.fecha_asignacion_unidad),
                justificacion: '',
                desvincular: false,
    ...
    const get_datos_vinculación = async (): Promise<void> => {
        ...
        if (persona.id_persona !== null && persona.id_persona !== undefined) {
            try {
                const response = await consultar_vinculacion_persona(persona.id_persona);
    ...
    const guardar_vinculacion = (): void => {
        ...
        const formulario: any = {
            ...
            fecha_a_finalizar_cargo_actual: vinculacion.fecha_finaliza.format("YYYY-MM-DD"),
                    ...
                    <Box component="form" sx={{ mt: '20px' }} noValidate autoComplete="off">
                        <Grid item container spacing={2}>
                            <Grid item xs={12} sm={4}>
                                ...
                                />
                                {(msj_error_cargo_actual !== "") && (<FormHelperText error >{msj_error_cargo_actual}</FormHelperText>)}
                            </Grid>
                            <Grid item xs={12} sm={4}>
                                <TextField
                                ...
                                />
                                {(msj_error_unidad_org !== "") && (<FormHelperText error >{msj_error_unidad_org}</FormHelperText>)}
                            </Grid>
                            <Grid item xs={12} sm={4}>
                                <TextField
                                    label="Fecha inicio"
                    ...
                    <Box component="form" sx={{ mt: '20px' }} noValidate autoComplete="off">
                        <Grid item container spacing={2}>
                            <Grid item xs={12} sm={4}>
                                ...
                                />
                                {(msj_error_fecha_asig !== "") && (<FormHelperText error >{msj_error_fecha_asig}</FormHelperText>)}
                            </Grid>
                            <Grid item xs={12} sm={4}>
                                <TextField
                                ...
                                />
                                {(msj_error_fecha_finaliza !== "") && (<FormHelperText error >{msj_error_fecha_finaliza}</FormHelperText>)}
                            </Grid>
                            <Grid item xs={12} sm={3}>
                                <Button
                                    color='primary'
                                    ...
                                    onClick={() => { set_modal_vincular_persona(true) }}
                                >
                                    Vincular
                                </Button>
