...
import { Button, Checkbox, Dialog, DialogActions, DialogContent, DialogTitle, Divider, FormControl, FormControlLabel, FormHelperText, Grid, TextField } from '@mui/material';
...
export const ActualizarCargo: React.FC<IProps> = ({ is_modal_active, set_is_modal_active, data_cargos, get_data }) => {
    ...
    const on_submit = async (data: EditarCargo): Promise<any> => {
        try {
            ...
            await editar_cargo(data_cargos.id_cargo, datos_cargos);
            set_is_modal_active(false);
                ...
                <DialogContent sx={{ mb: '0px' }}>
                    <Grid container spacing={1}>
                        ...
                        <Grid item xs={12}>
                            <FormControl
                                required
                                error={errors.activo?.type === 'required'}
                                component="fieldset"
                                variant="standard"
                            >
                                <FormControlLabel
                                ...
                                />
                                {errors.activo?.type === 'required' && (
                                    <FormHelperText>
                                        Debe enviar el estado del cargo
                                    </FormHelperText>
                                )}
                            </FormControl>
                        </Grid>
