...
export const CobroPersuasivo: React.FC<any> = ({
  ...
  useEffect(() => {
    if (id_fecha?.id_deudor && is_generate_resolucion) {
      data = [];
      if (data.length === 0) {
        const interesesCalculados = Math.round((0.12 * 30) * diasDiferencia * Number(id_fecha?.monto_inicial));
  
        data.push([
          id_fecha?.nombre,
          Math.round(Number(id_fecha?.monto_inicial)).toLocaleString('es-CO', { style: 'currency', currency: 'COP', maximumFractionDigits: 0 }),
          interesesCalculados.toLocaleString('es-CO', { style: 'currency', currency: 'COP', maximumFractionDigits: 0 }),
          Math.round(Number(id_fecha?.monto_inicial) + interesesCalculados).toLocaleString('es-CO', { style: 'currency', currency: 'COP', maximumFractionDigits: 0 })
        ]);
        sumaTotal = Math.round(Number(id_fecha?.monto_inicial) + interesesCalculados).toLocaleString('es-CO', { style: 'currency', currency: 'COP', maximumFractionDigits: 0 });
      }
      generatePDF();
    } 
    if (currentDeudor?.id_deudor && is_generate_resolucion && dataClean.length && !id_fecha?.id_deudor) {
      data = [];
      dataClean.forEach((item: any) => {
        if (item.id_deudor === currentDeudor.id_deudor) {
          const interesesCalculados = Math.round((0.12 * 30) * diasDiferencia * Number(item.monto_inicial));
  
          data.push([
            item.nombre,
            Math.round(Number(item.monto_inicial)).toLocaleString('es-CO', { style: 'currency', currency: 'COP', maximumFractionDigits: 0 }),
            interesesCalculados.toLocaleString('es-CO', { style: 'currency', currency: 'COP', maximumFractionDigits: 0 }),
            Math.round(Number(item.monto_inicial) + interesesCalculados).toLocaleString('es-CO', { style: 'currency', currency: 'COP', maximumFractionDigits: 0 })
          ]);
        }
      });
      const total1 = data.reduce((acc, curr) => acc + Number(curr[1].replace(/\D/g, '')), 0);
      const total2 = data.reduce((acc, curr) => acc + Number(curr[2].replace(/\D/g, '')), 0);
      data.push([
        'TOTAL',
        total1.toLocaleString('es-CO', { style: 'currency', currency: 'COP', maximumFractionDigits: 0 }),
        total2.toLocaleString('es-CO', { style: 'currency', currency: 'COP', maximumFractionDigits: 0 }),
        (total1 + total2).toLocaleString('es-CO', { style: 'currency', currency: 'COP', maximumFractionDigits: 0 })
      ]);
      sumaTotal = (total1 + total2).toLocaleString('es-CO', { style: 'currency', currency: 'COP', maximumFractionDigits: 0 });
  ...
  // }, [is_generate_resolucion, currentDeudor, dataClean, datos, aprobadores_data, diasDiferencia]);
}, []);
  ...
  //     data = [];
  //     if (data.length === 0) {
  //       const interesesCalculados = ((0.12 * 30) * diasDiferencia * Number(id_fecha?.monto_inicial)).toFixed(2);
  
  //       data.push([id_fecha?.nombre, id_fecha?.monto_inicial, interesesCalculados, (Number(id_fecha?.monto_inicial) + Number(interesesCalculados)).toLocaleString('es-CO', { maximumFractionDigits: 0 })]);
  //       sumaTotal = (Number(id_fecha?.monto_inicial) + Number(interesesCalculados)).toLocaleString('es-CO', { maximumFractionDigits: 0 });
  //     }
  //     generatePDF();
  //   } 
  //   if (currentDeudor?.id_deudor && is_generate_resolucion && dataClean.length && !id_fecha?.id_deudor) {
  ...
  //     dataClean.forEach((item: any) => {
  //       if (item.id_deudor === currentDeudor.id_deudor) {
  //         const interesesCalculados = ((0.12 * 30) * diasDiferencia * Number(item.monto_inicial)).toFixed(2);
  
  //         data.push([item.nombre, item.monto_inicial, interesesCalculados, (Number(item.monto_inicial) + Number(interesesCalculados)).toLocaleString('es-CO', { maximumFractionDigits: 0 })]);
  //       }
  //     });
  //     const total1 = data.reduce((acc, curr) => acc + Number(curr[1]), 0);
  //     const total2 = data.reduce((acc, curr) => acc + Number(curr[2]), 0);
  //     data.push(['TOTAL', total1.toLocaleString('es-CO', { maximumFractionDigits: 0 }), total2.toLocaleString('es-CO', { maximumFractionDigits: 0 }), (total1 + total2).toLocaleString('es-CO', { maximumFractionDigits: 0 })]);
  //     sumaTotal = (total1 + total2).toLocaleString('es-CO', { maximumFractionDigits: 0 });
  //     if (aprobadores_data?.send_data) {
  //       set_form_table_values({
  //         ...aprobadores_data,
  //         send_data: false
  ...
  //     data = [];
  //     if (data.length === 0) {
  //       // const interesesCalculados = Number(id_fecha?.monto_inicial) * Number(id_fecha?.valor_intereses);
  //       const interesesCalculados = (0.12 * 30) * diasDiferencia * Number(id_fecha?.monto_inicial);

  //       data.push([id_fecha?.nombre, id_fecha?.monto_inicial, interesesCalculados, (Number(id_fecha?.monto_inicial) + Number(interesesCalculados))]);
  //       sumaTotal = (Number(id_fecha?.monto_inicial) + interesesCalculados).toLocaleString('es-CO');
  //     }
  //     generatePDF();
  //   } 
  //   if (currentDeudor?.id_deudor && is_generate_resolucion && dataClean.length && !id_fecha?.id_deudor) {
  ...
  //     dataClean.forEach((item: any) => {
  //       if (item.id_deudor === currentDeudor.id_deudor) {
  //         // const interesesCalculados = Number(item.monto_inicial) * Number(item.valor_intereses);
  //         const interesesCalculados = (0.12 * 30) * diasDiferencia * Number(item.monto_inicial);

  //         data.push([item.nombre, item.monto_inicial, interesesCalculados, ((Number(item.monto_inicial) + interesesCalculados).toLocaleString('es-CO'))]);
  //       }
  //     });
  //     const total1 = data.reduce((acc, curr) => acc + Number(curr[1]), 0);
  //     const total2 = data.reduce((acc, curr) => acc + Number(curr[2]), 0);
  ...
  const generatePDF = () => {
    ...
    const docDefinition = {
      ...
      content: [
          ...
          columns: [
            {
              ...
              text: [
                ...
                { text: `\n`},
                { text: 'Señores\n', fontSize: 12 },
                { text: `${id_fecha?.id_deudor?.nombres || currentDeudor.nombre_completo} ${id_fecha?.id_deudor?.apellidos || ''}\n`, fontSize: 12 },
                { text: `CL 27 SUR 43-56\n`, fontSize: 12 },
                { text: `Teléfono: ${id_fecha?.id_deudor?.telefono || ''}\n`, fontSize: 12 },
                { text: [
                    { text: `E-mail: `, fontSize: 12 },
                    { text: `${id_fecha?.id_deudor?.email || currentDeudor.email}\n`, decoration: 'underline', color: 'blue', link: `emailto: ${id_fecha?.id_deudor?.email || currentDeudor.email}` }
                  ]
                },
              ...
              stack: [
                ...
                },
                {
                  ...
                  stack: [
                    ...
                    { text: `Nombre: ${id_fecha?.id_deudor?.nombres || currentDeudor.nombre_completo} ${id_fecha?.id_deudor?.apellidos || ''}`, alignment: 'left', fontSize: 9, margin: [ anchoPagina/2 + 30, 0, 0, 10 ] }
                  ],
                }
              ]
            }
        ...
        { text: `En atención al asunto de la referencia y revisados los registros contables de nuestra entidad, se evidencia que ${id_fecha?.id_deudor?.nombres || currentDeudor.nombre_completo} ${id_fecha?.id_deudor?.apellidos || ''}, identificado con C.C./NIT No. ${id_fecha?.id_deudor?.identificacion || currentDeudor.numero_identificacion} no ha cumplido con en el pago de las siguientes obligaciones:`, alignment: 'justify', fontSize: 12, margin: [0, 15, 0, 0] },
        {
          table: {
            headerRows: 2,
            widths: ['*', '*', '*', '*'],
...
//       data = [];
//       if (data.length === 0) {
//         const interesesCalculados = Math.round((0.12 * 30) * diasDiferencia * Number(datos.monto_inicial));
  
//         data.push([
//           datos.nombre,
//           Math.round(Number(datos.monto_inicial)).toLocaleString('es-CO', { style: 'currency', currency: 'COP', maximumFractionDigits: 0 }),
//           interesesCalculados.toLocaleString('es-CO', { style: 'currency', currency: 'COP', maximumFractionDigits: 0 }),
//           Math.round(Number(datos.monto_inicial) + interesesCalculados).toLocaleString('es-CO', { style: 'currency', currency: 'COP', maximumFractionDigits: 0 })
//         ]);
//         sumaTotal = Math.round(Number(datos.monto_inicial) + interesesCalculados).toLocaleString('es-CO', { style: 'currency', currency: 'COP', maximumFractionDigits: 0 });
//       }
//       generatePDF();
//     } 
...
//   //     data = [];
//   //     if (data.length === 0) {
//   //       const interesesCalculados = ((0.12 * 30) * diasDiferencia * Number(datos.monto_inicial)).toFixed(2);
  
//   //       data.push([datos.nombre, datos.monto_inicial, interesesCalculados, (Number(datos.monto_inicial) + Number(interesesCalculados)).toLocaleString('es-CO', { maximumFractionDigits: 0 })]);
//   //       sumaTotal = (Number(datos.monto_inicial) + Number(interesesCalculados)).toLocaleString('es-CO', { maximumFractionDigits: 0 });
//   //     }
//   //     generatePDF();
//   //   } 
//   //   if (currentDeudor?.id_deudor && is_generate_resolucion && dataClean.length && !datos?.id_deudor) {
...
//   //     data = [];
//   //     if (data.length === 0) {
//   //       // const interesesCalculados = Number(datos.monto_inicial) * Number(datos.valor_intereses);
//   //       const interesesCalculados = (0.12 * 30) * diasDiferencia * Number(datos.monto_inicial);

//   //       data.push([datos.nombre, datos.monto_inicial, interesesCalculados, (Number(datos.monto_inicial) + Number(interesesCalculados))]);
//   //       sumaTotal = (Number(datos.monto_inicial) + interesesCalculados).toLocaleString('es-CO');
//   //     }
//   //     generatePDF();
//   //   } 
//   //   if (currentDeudor?.id_deudor && is_generate_resolucion && dataClean.length && !datos?.id_deudor) {
