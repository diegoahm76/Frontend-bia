...
export const Documento: React.FC<BuscarProps> = ({
  is_modal_active_doc,
  set_doc,
}) => {
  ...
  const fetchPlanPago = async (): Promise<void> => {
    try {
      const url = `/recaudo/planes-pagos/validacion/`;
      const res = await api.get(url);
      const planPagoData: PlanPagoData = res.data?.data || {
        plan_pago: {},
        cuotas: [],
      };
  ...
  const columns: GridColDef[] = [
    ...
    },
    {
      ...
      width: 130,
      flex: 1,
      renderCell: (params) => `${params?.value} COP`,
    },
  ];

  const totalValorCapital = cuotas.reduce(
    (acc, cuota) => acc + parseFloat(cuota.valor_capital),
    0
  );
  const totalValorInteres = cuotas.reduce(
    (acc, cuota) => acc + parseFloat(cuota.valor_interes),
    0
  );
  const totalMontoCuota = cuotas.reduce(
    (acc, cuota) => acc + parseFloat(cuota.monto_cuota),
    0
  );
  ...
  const generarHistoricoBajas = () => {
    ...
    let tituloColumnas =
      'Nro Cuota  | Fecha Venc.    | Valor Capital   | Valor Interés   | Monto Cuota  ';
  ...
  const isButtonDisabled =
    !unidadSeleccionada || !selectedSerieSubserie || !formmmm.archivo;
