...
export const AutodeclaracionFormulario: React.FC = () => {
  ...
  const fetch_Crear_archivo_digital = async () => {
    try {
      ...
      formData.append(
        'captacionesMensualesAgua',
        JSON.stringify(
          Data.captacionesMensualesAgua.map((row, index) => ({
            ...row,
            id: index,
          }))
        )
      ...
      formData.append(
        'informacionFuentesAbastecimiento',
        JSON.stringify(
          Data.informacionFuentesAbastecimiento.map((row, index) => ({
            ...row,
            id: index,
          }))
        )
  ...
  const handleCurrentCaptacionChange = (event: {
  ...
  const verificarDatosCaptacion = () => {
    const campos = [
      ...
      {
        nombre: 'Volumen de agua captada',
        valor: currentCaptacion.volumenAguaCaptada,
      },
  ...
  const agregarCaptacion = () => {
    ...
    const mesYaExiste = Data.captacionesMensualesAgua.some(
      (captacion) => captacion.mes === currentCaptacion.mes
    );
    ...
    if (mesYaExiste) {
      ...
      setFormData((prevState) => ({
        ...prevState,
        captacionesMensualesAgua: [
          ...prevState.captacionesMensualesAgua,
          currentCaptacion,
        ],
      }));
  ...
  const eliminarcaptacion = (id: number) => {
    setFormData((prevState) => ({
      ...prevState,
      captacionesMensualesAgua: prevState.captacionesMensualesAgua.filter(
        (item, index) => index !== id
      ),
    }));
  };
  const columnas_captacion = [
    ...
    {
      field: 'volumenAguaCaptada',
      headerName: 'Volumen de agua Captada',
      width: 220,
    },
    { field: 'mes', headerName: 'Mes', width: 210 },
    {
      ...
      renderCell: (params: { id: any }) => {
        return (
          <IconButton
            color="error"
            onClick={() => eliminarcaptacion(params.id)}
          >
            <DeleteForeverIcon />
          </IconButton>
        );
  ...
  const handleCurrentFuenteChange = (event: {
    target: { name: any; value: any };
  }) => {
    const { name, value } = event.target;
  ...
  const verificarDatosFuente = () => {
    const campos: Campo[] = [
      ...
      {
        nombre: 'Caudal concesionado',
        valor: currentFuente.caudalConcesionado,
      },
      {
        nombre: 'Sistema de medicion de agua captada',
        valor: currentFuente.sistemaMedicionAguaCaptada,
      },
    ...
    for (let campo of campos) {
      if (
        campo.valor === null ||
        campo.valor === '' ||
        (typeof campo.valor === 'number' && isNaN(campo.valor))
      ) {
        return campo.nombre;
      }
    }
  ...
  const agregarFuente = () => {
    ...
    setFormData((prevState) => ({
      ...prevState,
      informacionFuentesAbastecimiento: [
        ...prevState.informacionFuentesAbastecimiento,
        currentFuente,
      ],
    }));
    setCurrentFuente({
  ...
  const eliminarFuente = (id: number) => {
    setFormData((prevState) => ({
      ...prevState,
      informacionFuentesAbastecimiento:
        prevState.informacionFuentesAbastecimiento.filter(
          (item, index) => index !== id
        ),
    }));
  };
  const columnas = [
    ...
    {
      field: 'sistemaMedicionAguaCaptada',
      headerName: 'Sistema de Medición',
      width: 220,
    },
    {
      field: 'acciones',
  ...
  const [formValues, setFormValues] =
    useState<SubZonaHidricaForm>(initialState);
  ...
  const updateget = () => {
    setCurrentFuente((prevData) => ({
      ...prevData,
      nombreFuente: formValues.id_tipo_agua_zona_hidrica,
    }));
    handle_close();
  };
            ...
            <Grid item xs={12} sm={12}>
              <FormControl required size="small" fullWidth>
                ...
                >
                  {rios.map((zona) => (
                    <MenuItem
                      key={zona.nombre_cuenca}
                      value={zona.nombre_cuenca}
                    >
                      {zona.nombre_cuenca}
                    </MenuItem>
                  ))}
      ...
      >
        ...
        <Grid item xs={12} sm={12} marginTop={2}>
          <DataGrid
            ...
            rows={Data.informacionFuentesAbastecimiento.map((row, index) => ({
              ...row,
              id: index,
            }))}
            getRowId={(row) => row.id}
      ...
      >
        ...
        <Grid item xs={12} sm={12} marginTop={2}>
          <DataGrid
            ...
            density="compact"
            rowsPerPageOptions={[10]}
            rows={Data.captacionesMensualesAgua.map((row, index) => ({
              ...row,
              id: index,
            }))}
            getRowId={(row) => row.id}
